...
`bmad-build` na **Estória 6.6** (Simulação de entrega) — a Épica 5 está inteira em `main`, e a 6.1 (`48f6723`), a 6.2 (`5f618db`), a 6.3 (`b6093e9`), a 6.4 (`081a60e`) e a 6.5 (`5b01aec`) também. A 6.6 chega com o esqueleto de pé: a simulação existe na varredura desde a 1.8, divide a casca `avancarPeloTempo` com a expiração desde a 5.11 e usa `AtorSimulacao` nas três linhas da tabela do AD-3; e a 6.4 deixou a colisão com o Administrador provada pelo lado dele (`TransicionarPeloAdministrador` trava e espera, a simulação usa `SKIP LOCKED` e pula). O que a estória fecha é a FR-33 inteira — intervalo configurável com 30 s em demonstração, o interruptor, nunca mover `CANCELADO` nem `AGUARDANDO_PAGAMENTO`, retomar do histórico depois de reiniciar, e `Consolidar` só no `ENVIADO` — com teste de cada condição, e não só o caminho feliz. A 5.11 está em `main` (`8a3dfd0`), a 5.10 em `main` (`22500bf`), a 5.9 em `main` (`577a6aa`), a 5.8 em `main` (`e421de8`), a 5.7
em `main` (`7cd26ac`), a 5.6 em `main` (`9174f88`), a 5.5 em `main` (`f3cc0ff`), a 5.4 em `main` (`6e5984f`), a 5.3 em `main` (`dbcb2b8`) e a 5.2 em
`main` (`400861b`), as dez em `review` no `sprint-status.yaml`, mais a 6.1, a 6.2, a 6.3, a 6.4 e a 6.5: falta a leitura humana
(`bmad-checkpoint-preview`) e o passeio da seção Verification das specs — 360 e 1440 px, só pelo
teclado —, que ninguém fez. **Nenhuma tela da Épica 5 nem da 6 foi aberta num navegador.** Da 6.5, o passeio é: um Produto de `,00`, cancelar o Pedido em `AGUARDANDO_PAGAMENTO` pela tela do Comprador, esperar ~5 s a aprovação chegar, e abrir `/admin/pedidos?status=CANCELADO` — o marcador "Pagamento aprovado" na linha dele e em nenhuma outra — e o Detalhe, com o `Alert` abaixo do Status e sem botão de fechar; a 1440 px e só pelo teclado. A tela aberta **antes** da aprovação chegar não a mostra sozinha: `CANCELADO` é terminal e a consulta em intervalo para, então é preciso recarregar. Da 6.4, o passeio é: entrar em `/admin/pedidos`, filtrar por `PAGO`, inverter a ordenação, recarregar conferindo que o estado se mantém na URL, e avançar um Pedido pelos três botões até `ENTREGUE` — com outra aba na Página de Produto para ver o Estoque total baixar no `ENVIADO`; depois, para a corrida, deixar a simulação mover o Pedido com a Tabela **filtrada** aberta e clicar no botão velho, para ver o `Alert` informativo dizendo em que Status ele está mesmo com a linha saindo da lista; e abrir o Detalhe de um Pedido para conferir o Comprador e o Vendedor de cada Item. Da 6.3, o passeio é a UJ-3: um Pedido em `SEPARANDO` → "Cancelar Pedido" → o `Dialog` nomeando o número → confirmar, e ver "Cancelado" na região viva e na linha do tempo, o foco no título, e outra aba com o Estoque de volta na Página de Produto; depois, o mesmo com o relógio correndo (`AGUARDANDO_PAGAMENTO`), e, para a corrida, cancelar numa tela `SEPARANDO` que a simulação já levou a `ENVIADO` e ver a frase "Este Pedido foi enviado enquanto você estava nesta tela…" em vez de erro. Da 6.2, o passeio é levar um total de `,00` de `PAGO` até "Entregue" pela simulação vendo a linha do tempo ganhar cada transição sem recarregar, a frase de entregue aparecer (desde a 6.3, junto do Status no topo, e não mais nos valores), o filete entre os marcadores alinhado, e nada rolando na horizontal a 360 px. Da 6.1, o passeio
...
A Épica 6 começou: 6.1 a 6.5 em main, em review — Meus pedidos, Detalhe,
cancelamento pelo Comprador, painel do Administrador, e o pagamento aprovado
sobre Pedido cancelado visível a ele.
Próximo passo: Estória 6.6 (Simulação de entrega).
```
