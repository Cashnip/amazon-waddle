...
`bmad-build` na **Estória 5.9** (Confirmação aprovada por webhook). A 5.8 está em `main` (`e421de8`), a 5.7
em `main` (`7cd26ac`), a 5.6 em `main` (`9174f88`), a 5.5 em `main` (`f3cc0ff`), a 5.4 em `main` (`6e5984f`), a 5.3 em `main` (`dbcb2b8`) e a 5.2 em
`main` (`400861b`), as sete em `review` no `sprint-status.yaml`: falta a leitura humana
...
- **a 5.9 encontra pronto:** a tela já consulta a cada 3 s e vira Detalhe no lugar quando o Status chega a `PAGO`,
  com a região `role="status"` anunciando. O que a 5.9 acrescenta é do lado do Go — inbox, estados terminais e a
  sinalização do aprovado sobre cancelado;
...
concorrência, o teste do NFR-7) e 5.8 (Tentativa de Pagamento e a tela do
Pedido em processamento) em main, as sete últimas em review.
Próximo passo: Estória 5.9 (Confirmação aprovada por webhook).
```
