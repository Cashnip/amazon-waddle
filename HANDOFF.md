...
`bmad-build` na **Estória 5.7** (Consistência de Estoque sob concorrência 🔒). A 5.6 está em `main`
(`9174f88`), a 5.5 em `main` (`f3cc0ff`), a 5.4 em `main` (`6e5984f`), a 5.3 em `main` (`dbcb2b8`) e a 5.2 em
`main` (`400861b`), as cinco em `review` no `sprint-status.yaml`: falta a leitura humana
(`bmad-checkpoint-preview`) e o passeio da seção Verification das cinco specs — 360 e 1440 px, só pelo
...
- **o que a 5.7 precisa saber antes de escrever o teste do NFR-7:** o contador por ano (`ProximoNumeroDoAno`)
  trava uma linha **antes** de `catalogo.Reservar`, então N criações paralelas correm em fila e nunca disputam o
  `ORDER BY id FOR UPDATE` ao mesmo tempo — o teste passaria sem provar o AD-5. A 5.7 decide entre mover a
  numeração para depois da Reserva ou disputar `catalogo.Reservar` diretamente. A mesma bancada prende os dois
  caminhos do duplo clique (`ON CONFLICT DO NOTHING` e a releitura em `recusar`) e o `TOTAL_DIVERGENTE` do passo
  5, que hoje só rodam quando a corrida acontece;
...
checkout) e 5.6 (criação do Pedido) em main, as cinco últimas em review.
Próximo passo: Estória 5.7 (consistência de Estoque sob concorrência).
```
