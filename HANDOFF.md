...
`bmad-build` na **Estória 5.10** (Pagamento recusado e nova Tentativa). A 5.9 está em `main` (`577a6aa`), a 5.8 em `main` (`e421de8`), a 5.7
em `main` (`7cd26ac`), a 5.6 em `main` (`9174f88`), a 5.5 em `main` (`f3cc0ff`), a 5.4 em `main` (`6e5984f`), a 5.3 em `main` (`dbcb2b8`) e a 5.2 em
`main` (`400861b`), as oito em `review` no `sprint-status.yaml`: falta a leitura humana
...
- **Três lentes só onde há raio de alcance real:** reserva de Estoque sob concorrência (5-6, 5-7), máquina de
  ...
  1.7, e o diff da estória era um ramo de log mais testes. O gatilho é o raio do **diff**, não o do tema.)*
...
checkout), 5.6 (criação do Pedido), 5.7 (consistência de Estoque sob
concorrência, o teste do NFR-7), 5.8 (Tentativa de Pagamento e a tela do
Pedido em processamento) e 5.9 (Confirmação aprovada por webhook) em main,
as oito últimas em review.
Próximo passo: Estória 5.10 (Pagamento recusado e nova Tentativa).
```
