...
- source_spec: `_bmad-output/implementation-artifacts/spec-7-2-o-diagrama-dos-seis-modulos-corresponde-ao-codigo.md`
  summary: A porta do AD-8 não tem `interface` Go. Hoje ela é a assinatura de `pagamento.IniciarTentativa` mais a rota `POST /api/v1/webhooks/pagamento`, e o adapter é `pagamento.Simulado` com `pagamento.EmitirConfirmacoesDevidas`. Para a 7.3 decidir se isso cumpre o AD-8 ou se a porta precisa virar tipo.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-7-2-o-diagrama-dos-seis-modulos-corresponde-ao-codigo.md`
  summary: A tabela do AD-6 na espinha põe os passos 2 (expirar) e 3 (simular a entrega) dentro de `pedido.Varrer` ("idem"), e o código tem três funções: `pedido.Varrer`, `pedido.Expirar` e `pedido.SimularEntrega`, esta só com `AZAMON_ENTREGA_SIMULACAO_ATIVA`. É da 7.3, que percorre o addendum e a espinha contra o código.
