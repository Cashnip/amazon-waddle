...
- source_spec: `_bmad-output/implementation-artifacts/spec-6-7-a-forma-de-exibicao-dos-sete-status.md`
  summary: RESOLVIDO — a entrada da 1.2 "O `Badge` do shadcn usa texto de 12px, e a regra de acessibilidade do `DESIGN.md` manda nada abaixo de 14px", para o selo de Status: por decisão humana de 2026-09-24, tomada na 6.7, o rótulo do selo conta como texto de conteúdo e sai em 14px.
  evidence: `aparenciaDoSelo` em `web/lib/pedido.ts` devolve o `className` inteiro do selo — `h-auto rounded-full text-sm` mais as classes do tom — e o `SeloDoStatus` o passa ao `Badge`, cujo `cn` faz o `text-sm` vencer o `text-xs` da base; `web/components/ui/badge.tsx` continua sem edição. O tom `falha` é preenchido (`bg-destructive text-white`, 4,76:1), porque o `destructive` do shadcn tinge a 10% e dá 3,99:1. `web/scripts/pedido.test.mjs` prende a aparência dos sete Status e dos desconhecidos. A decisão cobre só o selo: o marcador `outline` "Pagamento aprovado" da 6.5 continua no `Badge` sem sobreposição, em 12px, e a tensão da espinha segue aberta para ele e para qualquer outro `Badge` com texto de conteúdo.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-7-1-o-readme-leva-do-clone-ao-sistema-rodando.md`
  summary: RESOLVIDO — os dois comentários velhos de `api/rotas.go` (`POST /api/v1/pedidos` "da Página de Produto direto ao Pedido" e `GET /api/v1/pedidos` "o esboço que a 6.1 substitui") foram reescritos na 7.3, no percurso do addendum.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-7-2-o-diagrama-dos-seis-modulos-corresponde-ao-codigo.md`
  summary: RESOLVIDO — a porta do AD-8 é contrato, e não `interface` Go: `pagamento.IniciarTentativa` mais `POST /api/v1/webhooks/pagamento`, com o adapter em `pagamento.Simulado` e `EmitirConfirmacoesDevidas`. Decidido e emendado no lugar na 7.3, com memlog da arquitetura e o porquê no addendum §10.
  evidence: `internal/pagamento/pagamento.go:64` (`type Simulado struct`), `:135` (`IniciarTentativa`) e `:309` (`EmitirConfirmacoesDevidas`); nenhum `type … interface` no pacote. O Paradigma da espinha diz "portas-e-adaptadores só onde a costura é real" e o AD-8 diz "a porta tem duas operações e mais nada", sem dizer se a porta é tipo ou contrato. O diagrama da 7.2 descreve a porta pelas duas operações e não toma o lado; a spec da 7.2 põe a decisão na 7.3.

- source_spec: `_bmad-output/implementation-artifacts/spec-7-2-o-diagrama-dos-seis-modulos-corresponde-ao-codigo.md`
  summary: O `deck-banca.html` ao lado da espinha ainda desenha a porta como `ProvedorDePagamento`, nome que não existe no código, e a fronteira no banco sem `pedido.contador_numero`. É insumo da 7.4.
  evidence: `grep -c ProvedorDePagamento deck-banca.html` dá 1 e `grep -c contador_numero` dá 0, no diretório `architecture-azamon-2026-09-05/`. O deck é de 2026-09-06, anterior à 5.10 e ao contador por ano; a 7.2 corrigiu a espinha e o `DIAGRAMA-MODULOS.md` e, pela spec, não tocou no deck. Quem ensaiar os roteiros na 7.4 com o deck projetado vai mostrar à banca um nome que o código não tem.

- source_spec: `_bmad-output/implementation-artifacts/spec-7-2-o-diagrama-dos-seis-modulos-corresponde-ao-codigo.md`
  summary: RESOLVIDO — a tabela do AD-6 nomeia as três funções de `pedido` que o relógio chama (`Varrer`, `Expirar`, `SimularEntrega`, esta só com `AZAMON_ENTREGA_SIMULACAO_ATIVA`), emendada no lugar na 7.3.
  evidence: `cmd/azamon/main.go:173-186` chama as três em sequência e depois `pagamento.EmitirConfirmacoesDevidas`. A ordem do AD-6 bate; o nome dos donos dos passos 2 e 3, não. A 7.2 corrigiu o grafo do AD-1 e a tabela de travessias do `DIAGRAMA-MODULOS.md`, e deixou o AD-6 fora do escopo.

- source_spec: `_bmad-output/implementation-artifacts/spec-7-2-o-diagrama-dos-seis-modulos-corresponde-ao-codigo.md`
  summary: O subteste "existem só as catorze tabelas do esqueleto" de `db/schema_test.go` não confere o schema `carrinho`, e o "as convenções de coluna do AD-2 valem em toda coluna" também não: `schemasDeModulo` (`db/schema_test.go:36`) lista quatro dos cinco schemas.
  evidence: `carrinho.carrinho` e `carrinho.item_carrinho` existem desde `20260918160000_carrinho_schema.sql`, e nenhum subteste de `db/schema_test.go` os nomeia. Uma tabela a mais em `carrinho`, ou uma coluna fora das convenções, passaria calada. A consulta "nenhuma chave estrangeira cruza schema" não filtra por schema e cobre `carrinho`. Fechar é pôr `carrinho` na lista e as duas tabelas no `quer`, e trocar "catorze" por "dezesseis"; a spec da 7.2 proíbe tocar código fora de `internal/fronteira_test.go`, então o `DIAGRAMA-MODULOS.md` diz a cobertura real.

- source_spec: `_bmad-output/implementation-artifacts/spec-7-3-o-addendum-percorrido-contra-o-codigo.md`
  summary: O AD-18 da espinha diz que a consulta em intervalo existe em "exatamente três superfícies e em nenhuma outra", e o Detalhe do Pedido do Administrador é a quarta: consulta `GET /api/v1/admin/pedidos/{id}` a cada 10 s enquanto o Pedido não é terminal. E o desempate: o AD-18 diz "sempre termina em `id` ascendente", e Meus pedidos (6.1) desempata em `p.id DESC`, de propósito (`internal/pedido/db/consultas.sql:210-228`). Emendar o AD-18 no lugar, com memlog da arquitetura. Sem dono nomeado: é contradição na espinha canônica e tem de fechar antes da entrega (SM-7); a 7.4, a última estória da épica, é o prazo.
  evidence: `web/app/admin/pedidos/[id]/detalhe-admin.tsx:84-90`, desde a 6.4 (`081a60e`); `ARCHITECTURE-SPINE.md`, AD-18. O percurso da 7.3 registrou a troca e o porquê no addendum §10, mas a spec da 7.3 só emendava o AD-4, o AD-6 e o AD-8.

- source_spec: `_bmad-output/implementation-artifacts/spec-7-3-o-addendum-percorrido-contra-o-codigo.md`
  summary: A Semente Estrutural da espinha diz de `catalogo.go` "interface pública — o ÚNICO arquivo que outro módulo importa", e a porta de cada módulo é o pacote: outro módulo chama símbolos de arquivos irmãos, como `identidade.BuscarEndereco` (`endereco.go`) e `catalogo.ErrVendedorComProdutos` (`vendedor.go`). Emendar a linha da árvore, com memlog. Sem dono nomeado: é contradição na espinha canônica e tem de fechar antes da entrega (SM-7); a 7.4, a última estória da épica, é o prazo.
  evidence: `ARCHITECTURE-SPINE.md`, Semente Estrutural (`catalogo.go  # interface pública — o ÚNICO arquivo que outro módulo importa`); `internal/pedido/pedido.go:167` chama `identidade.BuscarEndereco`. Os cabeçalhos dos seis módulos, que diziam o mesmo, foram corrigidos na 7.3; a espinha ficou fora do escopo dela.

- source_spec: `_bmad-output/implementation-artifacts/spec-7-3-o-addendum-percorrido-contra-o-codigo.md`
  summary: A espinha descreve quatro mecanismos de um jeito que o código não segue, achados no segundo sentido do percurso da 7.3: o AD-10 fala em `next/image` com `unoptimized` e proíbe `fetch` do servidor Next para o Go dentro de RSC, e a Vitrine e a Página de Produto fazem esse `fetch` (1.5, 3.5) e nenhuma tela usa `next/image`; o AD-19 diz que o predicado é exposto por `catalogo.Visiveis` e a VIEW é construída sobre ele, e no código a VIEW `produto_visivel` é a fonte e `Visiveis` a lê; a assinatura de `Disponivel` na espinha (`Disponivel(ctx, produtoIDs []uuid) map[uuid]int`) não tem banco nem erro, e a do código recebe `DBTX` e devolve `(map[string]…, error)`; e a tabela de posse de rotas do AD-16 não tem `GET /api/v1/categorias`. Emendar no lugar, com memlog da arquitetura. Sem dono nomeado; prazo: antes da entrega (SM-7).
  evidence: `web/app/page.tsx:16-17,56-78`; `web/components/imagem-do-produto.tsx:6-9`; `db/migracoes/20260918120000_catalogo_produto_visivel.sql:1-16`; `internal/catalogo/catalogo.go:99-131`; `api/rotas.go:52-55`. O addendum registra cada um no bloco da estória que o decidiu (3.1, 3.2–3.3, 3.4, 3.5, 3.7–3.9), marcado "registrado na 7.3"; a espinha ficou fora do escopo da 7.3, que só emendava o AD-4, o AD-6 e o AD-8.

- source_spec: `_bmad-output/implementation-artifacts/spec-7-3-o-addendum-percorrido-contra-o-codigo.md`
  summary: O teto de unidades por Item de Carrinho existe em dois lugares: `AZAMON_CARRINHO_UNIDADES_MAX` no Go (Épica 4), que decide, e `TETO_POR_ITEM = 10` literal em `web/lib/quantidade.ts` (3.6), que limita o seletor da Página de Produto. Mudar a variável sem mudar a constante deixa o seletor prometer o que o Go recusa, ou esconder o que ele aceita. Fazer o `web/` ler o teto do Go (o `GET /api/v1/carrinho` já leva o limiar do Frete no mesmo molde) ou prender os dois num teste.
  evidence: `internal/plataforma/config.go:108`; `web/lib/quantidade.ts:1-6`; `web/app/produtos/[id]/caixa-de-compra.tsx:125`. O comentário de `quantidade.ts` prometia a troca desde a 3.6 e foi corrigido na 7.3 para dizer que os dois têm de mudar juntos.
