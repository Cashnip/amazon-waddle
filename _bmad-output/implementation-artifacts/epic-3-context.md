...
- **Posse das rotas (exaustiva):** `GET /api/v1/produtos` é de `busca`, com ou sem termo, filtros, ordenação e paginação. `GET /api/v1/produtos/<id>` e `/api/v1/admin/...` (CRUD e Estoque) são de `catalogo`. `catalogo` não registra listagem nem paginação. Se dois módulos registrarem o mesmo padrão no `ServeMux`, o binário entra em pânico no arranque.
