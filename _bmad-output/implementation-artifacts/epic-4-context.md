...
- **Esvaziamento pelo Pedido:** `pedido.Criar` chama `carrinho.Esvaziar(ctx, tx, compradorID, itemIDs)` na mesma transação, depois que `Reservar` dá certo. É o único ponto em que outro módulo escreve no Carrinho. Cancelar um Pedido não devolve nada ao Carrinho. O esvaziar que o Comprador confirma na tela (FR-18) é outro fim: `DELETE /api/v1/carrinho/itens` e `carrinho.Limpar`, de nomes distintos de propósito (leitura do AD-3, sem emenda).
- **Recusa por Estoque no Carrinho (AD-5, AD-14):** 409 `ESTOQUE_INSUFICIENTE`, `dados` com `produto_id`/`disponivel`/`solicitado`, mensagem nomeando o Produto; confere a soma resultante do Item, não só o acréscimo. É conselho, não reserva.
